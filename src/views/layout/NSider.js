...
class MySiderContainer extends React.PureComponent {
    ...
    initMenu = (pathname) => {
        ...
        if (name) {
            let parentKeys = getParentMenusByName(this.props.openAccessMenu, name).map(item => {
                return item.name;
            });
    ...
    openMenu = v => {
        let parentKeys = getParentMenusByName(this.props.openAccessMenu, v[v.length - 1]).map(item => {
            return item.name;
        });
        this.setState({
            openKeys: parentKeys
